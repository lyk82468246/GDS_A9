        ...
        while True:
            try:
                ...
                if not self.is_started and GPIO.input(self.start_button_pin) == GPIO.LOW:
                    ...
                    self.reference_right_distance = min([scan_data.get(a, (10000, 0))[0] for a in range(0, 30) if scan_data.get(a, (0, 0))[0] > 0], default=10000)
                ...
                if self.is_started:
                    ...
                    if self.is_initialized:
                        speed_adjustment = (self.reference_right_distance - right_distance) / 1000.0
                        adjusted_speed = self._map_speed(self.forward_speed + speed_adjustment * 50, 0, 100, self.min_speed, self.max_speed)
                        self.car.move(90, adjusted_speed)
                        print(f"调整速度: {adjusted_speed}")
                    else:
                        self.is_initialized = True
                        print(f"小车已初始化,开始保持右侧距离: {self.reference_right_distance}mm")

                    # 检查是否已越过中线
                    if right_distance < self.min_distance_to_midline and not self.is_crossing_midline:
                        self.is_crossing_midline = True
                        print("检测到已越过中线,开始扫描敌方小车...")
